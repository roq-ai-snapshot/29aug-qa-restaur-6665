...
import { getTableReservations } from 'apiSdk/table-reservations';
...
function DashboardPage() {
  ...
  const tableReservationStat = useSWR<PaginatedInterface<TableReservationInterface>>(
    () => `/table-reservations`,
    () => getTableReservations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: tableReservationStat?.data?.totalCount,
